...
void add_to_value_node(void* value, HashNode* temp_node){
    ...
    temp_value_node->next = (struct ValueNode *) new_value_node;
    new_value_node->value = value;
    new_value_node->next = NULL;
}


void add_new_hash_node_in_the_middle(HashNode* temp_node, void* value, Manager* manager) {
    //if there's only one HashNode
    if(temp_node->next == NULL){
        HashNode* new_node = malloc(sizeof(HashNode));
        temp_node->prev = (struct HashNode *) new_node;
        new_node->prev = NULL;
        new_node->next = (struct HashNode *) temp_node;
    ...
    //if temp node is head
        ...
        HashNode* new_node = malloc(sizeof(HashNode));
        HashNode* prev_node = (HashNode *) temp_node->prev;
        prev_node->next = (struct HashNode *) new_node;
        new_node->prev = (struct HashNode *) prev_node;
        new_node->next = (struct HashNode *) temp_node;
        temp_node->prev = (struct HashNode *) new_node;
        new_node->value_head = malloc(sizeof(ValueNode));
        new_node->value_head->next = NULL;
...
void add_new_hash_node_at_the_end(HashNode* temp_node, void* value){
    HashNode* new_node = malloc(sizeof(HashNode));
    temp_node->next = (struct HashNode *) new_node;
    new_node->prev = (struct HashNode *) temp_node;
    new_node->next = NULL;
    new_node->value_head = malloc(sizeof(ValueNode));
    new_node->value_head->next = NULL;
    new_node->value_head->value = value;
...
void* get(Manager* manager, void* hash_value){
    ...
    while(temp_node != NULL){
        ...
        int f_comp_result = manager->f_comp(hash_value, value_hash_ptr);
        free(value_hash_ptr);
        if(f_comp_result == 0){
            return temp_node->value_head->value;
        }
   ...
   if(manager->head == NULL){
       ...
       while(temp_node->next != NULL){
           ...
           //if current hash value is greater
               ...
               add_new_hash_node_in_the_middle(temp_node, value,manager);
               return;
           }
           temp_node = (HashNode *) temp_node->next;
       }
...
void clear(Manager* manager){
    ...
    while(temp_node != NULL){
        ValueNode* temp_value_node = temp_node->value_head;
        while(temp_value_node!= NULL){
            ValueNode* next_value_node = (ValueNode *) temp_value_node->next;
            free(temp_value_node);
            temp_value_node = next_value_node;
        }
        HashNode* next_hash_node = (HashNode *) temp_node->next;
        free(temp_node);
