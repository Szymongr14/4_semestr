...
if isfield(energy, country) && isfield(energy.(country), source)
    ...
    for i = 1:length(degrees)
        ...
        y_approximation{i} = idct2_custom(X, degrees(i), N, P);
        y_approximation_coarse = idct2_custom(X, degrees(i), N, N);
        
        squared_errors = (y_approximation_coarse - y_yearly).^2;
        mse(i) = mean(squared_errors);
        if i > 1
            squared_errors = (y_last - y_approximation{i}).^2;
            msek(i-1) = mean(squared_errors);
        end    
        y_last = y_approximation{i};
    end
    mse = mse';
    msek = msek';
else
...
xlabel('x');
ylabel('y');
legend('original', '1', '2', '3', '4', 'Location', 'best');
hold off

subplot(3, 1, 2);
semilogy(1:length(degrees), mse);
title('Mean square error');  
xlabel('polynomial degree');
ylabel('MSE value');

subplot(3, 1, 3);
semilogy(1:length(msek), msek);
